...
As you learned in [Prepare a local relay chain](/tutorials/connect-relay-and-parachains/prepare-a-local-relay-chain/), a parachain can work with a single collator.
...
You can **modify** the provided **plain** [relay chain spec file](/tutorials/connect-relay-and-parachains/prepare-a-local-relay-chain/#pre-configured-chain-spec-files) to include more validators, or go the more "correct" route used for production of modifying the **source** for genesis state in `chain_spec.rs` for your **relay chain** to add more testnet validators at genesis.
...
- [Prepare a local relay chain](/tutorials/connect-relay-and-parachains/prepare-a-local-relay-chain/) tutorial.
