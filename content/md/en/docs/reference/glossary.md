...
The pointer to a parent block is a hash of the parent and the tree is pruned into a list using a [fork-choice rule](/ls/consensus#fork-choice-rules) and an optional [finality](#finality) mechanism.
...
Consensus is comprised of [authorship](#author), [finality](#finality), and [fork-choice rule](/ls/consensus#fork-choice-rules). 
...
- Cryptographic schemes identify and authenticate the [accounts](/ls/account-addresses-keys) used to perform [transactions](#transaction) in the Substrate runtime.
...
For information about how the database backend is implemented and used by Substrate-based chains, see [Runtime storage](/ime-storage).
...
- signed or unsigned [transactions](/ls/transaction-types).
...
[chain specification](/n-specs) file.
...
The [metadata](/tend) that exposes information about a Substrate [blockchain](#blockchain) enables you to interact with that system. 
...
The FRAME `system` module defines three built-in [origins](/ins#raw-origins). 
...
This trie is exposed to the [runtime](#runtime) as [a simple key/value map](/ls/state-transitions-and-storage) where both keys and values can be arbitrary byte arrays.
...
[runtime](#runtime). 
Learn more about storage items in this article about [runtime storage](/ime-storage).
...
A type of [extrinsic](#extrinsic) that can be safely gossiped between [nodes](#node) on the network because it can be verified through [signatures](/ls/transaction-types) or [signed extensions](/reference/transaction-format#signed-extension).
