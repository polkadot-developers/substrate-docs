...
Once your node is running, use the Polkadot JS Apps' `RPC calls` under the `Developer` tab to query `eth > getBalance(address, number)` with Alice's EVM account ID (`0xd43593c715fdd31c61141abd04a99fd6822c8558`).
...
![architecture diagram](/media/images/docs/tutorials/evm-ethereum/pallet-evm.png)

### Ethereum pallet

The Ethereum pallet is responsible for storing Ethereum-formatted blocks, transaction receipts, and transaction statuses.

![architecture diagram](/media/images/docs/tutorials/evm-ethereum/pallet-ethereum.png)

### Wrapping Ethereum transactions

When a user submits a raw Ethereum transaction, we need to convert it into a Substrate transaction.
The conversion is simple.
We just wrap the raw transaction in a call the `pallet_ethereum`'s `transact` extrinsic.
This is done in the runtime.

Note that Ethereum Accounts and Substrate accounts in this template are not directly compatible for using keys.
For an explainer on this, please see the [Moonbeam documentarian on EVM and Substrate Accounts](https://docs.moonbeam.network/learn/unified-accounts/#substrate-evm-compatible-blockchain)
...
/media/images/docs/tutorials/evm-ethereum/block-import.png
![architecture diagram](/media/images/docs/tutorials/evm-ethereum/rpc.png)
...
![architecture diagram](/media/images/docs/tutorials/evm-ethereum/block-import.png)
...
The storage slot was calculated using [a provided utility](https://github.com/paritytech/frontier/blob/master/template/utils/README.md#--erc20-slot-slot-address).
...
In this case, the arguments correspond to Bob's EVM account ID (`0x8eaf04151687736326c9fea17e25fc5287613693`) and the number of tokens to be transferred (`0xdd`, or 221 in hex).
