...
The `node-authorization` pallet uses an [offchain worker](/ls/offchain-operations)
...
1. Compile the node template by running the following command:
   ...
   If you encounter issues when you compile, you can try the troubleshooting tips in [Troubleshoot Rust issues](/oubleshoting/).
...
1. Add the `pallet-node-authorization/std` features to the list of `features` to enable when compiling the runtime.
   ...
   For more detailed information about how the runtime is compiled as a native Rust binary with the standard library and as a WebAssembly binary using the `no_std` attribute, see [Building the runtime](/d-runtime/).
...
The `node-authorization` pallet uses an [offchain worker](/ls/offchain-operations) to configure node connections.
...
- [Accounts, addresses, and keys](/ls/accounts-addresses-keys)