...
Once the parachain is active as a parathread, the related project team should [open a request](https://github.com/paritytech/subport/issues/new?assignees=&labels=Rococo&template=rococo.yaml) for either a **permanent** or a **temporary parachain slot**.
...
- You are familiar with registering and reserving your unique para ID.
...
The easiest way to register your chain as a parathread is by using Polkadot JS apps.
...
Presently, Rococo's runtime sets `AssignSlotOrigin` to `EnsureRoot<Self::AccountId>;`, meaning that also someone with sudo access can assign slots.

The Rococo `sudo` key is controlled by Parity Technologies at this time.
To request a slot, you need to go to the [Subport repo and open a `Rococo Slot Request`](https://github.com/paritytech/subport/issues/new?assignees=&labels=Rococo&template=rococo.yaml) once you've gone through the above steps and are ready to connect.
Assuming you've submitted your genesis state and genesis runtime Wasm, a Parity team member will initiate the slot for you.
...
1. Insert your reserved para ID (make sure this matches the one you reserved from the previous section) and select `Current` for the `LeasePeriodStart`.
...
If you want a more broad audience to access have access to your parachain, having public RPC node(s) available to connect to is required.
...
You will need to get the word out about these by publishing your endpoints for substrate clients like the Polkadot-JS [`apps`](https://github.com/polkadot-js/apps/) User Interface (UI) to connect to your parachain.
...
Once merged, this will have your parachain appear in the listing of available networks under the Rococo section of the `Network Selection` navigation. It optionally can also include style just for your network as well.
...
For reference, here are a few examples of PRs adding Rococo parachains to the Apps UI:
...
With your slot activated, you're now able to test your parachain on the Rococo test net.
