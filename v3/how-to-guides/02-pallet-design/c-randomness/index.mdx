...
1. Import `Randomness`
  ...
  In the pallet you want to use, import the [`Randomness`](/rustdocs/latest/frame_support/traits/trait.Randomness.html) 
...
1. Create a nonce.
  ...
  * **Create a private nonce function.** This will be used to increment the nonce each time it's used.
  ...
  The `increment_nonce()` private function can be implemented in such a way that it both returns and updates 
  the nonce. For example:
    ...
    <Message 
      ...
      title={`Note`}
      text=
      "To learn more about the wrapping and encoding methods, see [`wrapping_add`](/rustdocs/latest/funty/trait.IsInteger.html#tymethod.wrapping_add) 
      and [`encode()`](/rustdocs/latest/frame_support/dispatch/trait.Encode.html#method.encode) in the Rust documentation.
      "
    />
...
4. Update your pallet's runtime implementation
