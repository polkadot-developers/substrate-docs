...
<Objectives
  data={[
    ...
    },
    {
      title: 'Overview',
      description:
        `Loose coupling is a technique that enables re-using logic from another pallet inside a pallet. In this guide
...
We want to use the [`Currency` trait](https://substrate.dev/rustdocs/latest/frame_support/traits/tokens/currency/trait.Currency.html) so 
...
In the above snippet, we're using [`total_issuance`](https://substrate.dev/rustdocs/latest/frame_support/traits/tokens/currency/trait.Currency.html#tymethod.total_issuance) 
...
that [implements the `Currency` trait](https://substrate.dev/rustdocs/latest/pallet_balances/index.html#implementations-1).
...
- `try_origin` from the [`EnsureOrigin`](https://substrate.dev/rustdocs/latest/frame_support/traits/trait.EnsureOrigin.html) trait 
...
- [Pallet Coupling]((/runtime/pallet-coupling))
