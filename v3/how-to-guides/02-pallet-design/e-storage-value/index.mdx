...
Creating a struct of similarly grouped storage items is an orderly way to keep track of them. When grouped in this way, it is easier to reference them than it is if individual \`StorageValue\` items are kept separately.
This can make testing and genesis configuration easier.

This guide will show you how to create a [\`StorageValue\`](/rustdocs/latest/frame_support/storage/trait.StorageValue.html) struct and use it in \`on_initialize\`. This struct does the following:
...
*need some kind of knowledge base info here*
