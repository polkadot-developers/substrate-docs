...
# Smart contracts and your blockchain

This guide will show you how to add the [Contracts pallet](/rustdocs/latest/pallet_contracts/) to your runtime so that your blockchain
to support Wasm smart contracts. You can follow similar patterns to add additional FRAME pallets to your runtime, however you should note that each pallet is a little different in terms of the specific configuration settings needed to use it correctly.
...
2. Update `runtime/Cargo.toml` and `node/Cargo.toml` with the following entries:
...
2. Add parameter types.
  ...
  Add them directly above `impl pallet_contracts::Config for Runtime`.
...
<Message
  ...
  the Contracts pallet to make calls to your node's storage without making a transaction, 
  you'll use two other pallets: the Contracts RPC Runtime API pallet and the Contracts RPC pallet.`}
/>
...
4. Implement the Contracts runtime API.
...
Build and run it usingthe following commands:
