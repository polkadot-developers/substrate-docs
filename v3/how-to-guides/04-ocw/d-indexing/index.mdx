...
Another way of handing data from on-chain to off-chain worlds is by saving it in the local storage of a noce via **off-chain indexing**.
...
The consequence is that in case non-unique keys are used, the data might be overwritten by different forked blocks and the content in the local stroage will be different between nodes.
