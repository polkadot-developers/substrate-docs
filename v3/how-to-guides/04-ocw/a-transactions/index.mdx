...
7. Implement the `CreateSignedTransaction` trait in the runtime.
  ...
  You can see a full example of the code in the [**Substrate** code base](https://github.com/paritytech/substrate/blob/monthly-2021-12/bin/node/runtime/src/lib.rs#L960-L999).
...
8. Implement `SigningTypes` and `SendTransactionTypes` in the runtime to support submitting  transactions, whether they are signed or unsigned.
  ...
  You can see an example of this implementation in the [**Substrate** code base](https://github.com/paritytech/substrate/blob/monthly-2021-12/bin/node/runtime/src/lib.rs#L1001-L1012).

9. Inject an account for this pallet to own. In developemnt environment
...
You can see a full example in [`pallet-example-offchain-worker` in **Substrate** code base](https://github.com/paritytech/substrate/blob/monthly-2021-12/frame/examples/offchain-worker).
...
You can refer to [Sending unsigned transactions](#sending-unsigned-transactions) for information about implementing the ValidateUnsigned trait and adding the ValidateUnsigned type to the runtime. 
...
To prepare the data structure to be signed, implement the [`SignedPayload` trait](https://paritytech.github.io/substrate/latest/frame_system/offchain/trait.SignedPayload.html).
  For example:
  ...
  You can also see an example [here](https://github.com/paritytech/substrate/blob/monthly-2021-12/frame/examples/offchain-worker/src/lib.rs#L344-L357).

2. In the pallet off-chain worker function, call the signer, then the function to send the transaction.
  ...
  The second function closure returns the on-chain call with payload and signature passed in. This call returns an
  `Option<(Account<T>, Result<(), ()>)>` result type to allow for the following results:
...
You can implement more complex logic for `ValidateUnsigned` trait, if needed.
  For example, to check whether a provided signature matches the public key used to sign the payload, you can implement the `ValidateUnsigned` trait using code similar to the following:
  ...
  This example uses [`SignedPayload`](https://paritytech.github.io/substrate/latest/frame_system/offchain/trait.SignedPayload.html?search=SignedPayload) to verify that the public key in the payload has the same signature as the one provided.
...
You can see a full example in the [off-chain function call](https://github.com/paritytech/substrate/blob/monthly-2021-12/frame/examples/offchain-worker/src/lib.rs#L505-L532) and [`ValidateUnsigned` implementation](https://github.com/paritytech/substrate/blob/monthly-2021-12/frame/examples/offchain-worker/src/lib.rs#L308-L318).
