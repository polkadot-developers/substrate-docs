...
9. Inject an account for this pallet to own. 
  In a developemnt environment (node running with `--dev` flag), this account key is inserted in the `node/src/service.rs` file as follows:
...
Refer to the [off-chain function call](https://github.com/paritytech/substrate/blob/monthly-2021-12/frame/examples/offchain-worker/src/lib.rs#L505-L532) and [the implementation of `ValidateUnsigned`](https://github.com/paritytech/substrate/blob/monthly-2021-12/frame/examples/offchain-worker/src/lib.rs#L308-L318) for a working example of the above.
