...
## Steps for sending signed transactions
...
5. From pt #1, we have configured so that for the runtime to implement this pallet `Config` trait, it also has to imeplement the [`CreateSignedTransaction` trait](https://paritytech.github.io/substrate/latest/frame_system/offchain/trait.CreateSignedTransaction.html), so let's implement that trait in our runtime. Looking at its rustdoc, we only need to implement the function `create_transaction()`.
  ...
  The above code seems long, but what it does is simple. It:

    - creates / prepares a `SignedExtra` type, and putting various checkers in-place.
    - creates a raw payload based on passed in `call` and `extra`.
    - signs the raw payload with the account public key.
    - finally, it bundles all data up and return a tuple of the call, the caller, the signature, and any signed extension data.

  A full exmaple code can be seen [here](https://github.com/jimmychu0807/substrate-offchain-worker-demo/blob/v2.0.0/runtime/src/lib.rs#L306-L341).

6. The runtime also needs to implement `SigningTypes` and `SendTransactionTypes` in order to submit transactions, whether signed or unsigned.
...
7. Now we need to configure which account does this pallet owned. In developemnt environment (node starting with `--dev` flag), the key is [inserted in `node/src/service.rs`](https://github.com/jimmychu0807/substrate-offchain-worker-demo/blob/v2.0.0/node/src/service.rs#L87-L105) at the following:
...
## Steps for sending unsigned transactions
...
5. We will need to implement `SendTransactionTypes` trait for the runtime. This is the same as the implementation used for sending signed transactions in pt #6 above.

## Steps for sending unsigned transactions with signed payload
...
You can refer to the exact instructions in the above. Let's see the major differences now.

1. Now within the off-chain worker function, we will prepare the data structure for the signed payload, called `Payload`. This payload need to implement [`SignedPayload` trait](https://paritytech.github.io/substrate/latest/frame_system/offchain/trait.SignedPayload.html). Let's use a simple example as follows:
...
2. Now in our `offchain_worker` entrance function, we can send the transaction this way:
  ...
  In the above code snippet, `send_unsigned_transaction()` accepts two function closures. The first closure returns the payload to be used. The second closure return the on-chain call with payload and signature passed in. This call returns a `Option<(Account<T>, Result<(), ()>)>` result type. They mean:

    * `None`: no account is available for sending transaction
    * `Some((account, Ok(())))`: transaction is successfully sent
    * `Some((account, Err(())))`: error occured when sending the transaction

3. Lastly, you may have a more complex `ValidateUnsigned` logic to check the provided signature does match with the public key signing the payload. You can easily do that with the following logics in the pallet.
  ...
  We have seen most of the above code before. The new code is that we use [`SignedPayload`](https://paritytech.github.io/substrate/latest/frame_system/offchain/trait.SignedPayload.html?search=SignedPayload) to verify the payload, which include the public key, has the same signature as the one provided.

Whew! This is a long chapter. But by now you should be able to use off-chain workers to send:
