...
Declare your pallet and the items it exposes in `construct_runtime!`, including the additional `Module` and `Storage`
...
Additionally, you must include relevant parameter types and constants if relevant. See the examples on how [`pallet_timestamp`][timestamp-frame]
is declared.
