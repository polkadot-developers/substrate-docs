...
This optional step we can leverage an _existing_ plain chain specification for a network that otherwise would require modification of te _source_ of the node to run on a _new network_.
...
It is _conventional_ to include the chain specification files for your node _within the source code itself_ so that anyone building your node and avoid any chance of non-determinism causing nodes to believe they are on different networks (as the genesis blob may differ).
