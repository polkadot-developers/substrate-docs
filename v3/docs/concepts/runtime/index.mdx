...
"[state transition function](../../getting-started/glossary#state-transition-function-stf)";
it is where Substrate developers define the storage items that are used to represent the
blockchain's [state](../../getting-started/glossary#state) as well as the
[functions](../extrinsics) that allow blockchain users to make
changes to this state.

In order to provide its defining forkless runtime upgrade capabilities, Substrate uses runtimes that
are built as [WebAssembly (Wasm)](../../getting-started/glossary#webassembly-wasm)
...
The core Substrate codebase ships with [FRAME](../../runtime/frame), Parity's system
...
[pallets](../../runtime/pallets). Each pallet encapsulates domain-specific logic that is expressed as a set of a
[storage items](../../runtime/storage), [events](../../runtime/events),
[errors](../../runtime/errors) and
[dispatchable functions](../../getting-started/glossary#dispatch). FRAME developers
can [create their own pallets](../../runtime/pallets) and reuse existing pallets,
including [over 50 that ship with Substrate](../../runtime/frame#prebuilt-pallets).
