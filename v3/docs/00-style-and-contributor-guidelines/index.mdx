...
You are encouraged to use [prettier](https://prettier.io/) and and IDE that supports using the
included `.editorconfig` file that will in real time help format your writing to match our style in
this repository.

> Please run `yarn fmt:docs` on **all PRs** to the upstream repository to have `prettier` format
> your content for you automatically.
