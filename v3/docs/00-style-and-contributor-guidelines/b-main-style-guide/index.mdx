...
contributors to be aware of available in the [Build and Deploy](#build-and-deploy) section.
...
Some gotchas or adding or editing new content specific to this repository:
...
- All `/rustdoc/` internal links must end with `.html` or `.html#some-ID`.
  Reasoning in [#425](https://github.com/substrate-developer-hub/substrate-docs/issues/425).
- From time to time, the development server local casche becomes corrupted. To fix this in a one liner:
    ```bash
    yarn clean && yarn dev
    ```
  ***PLEASE do this when reviewing a page before every PR*** - this ensures your state is what the
  build CI see as well.  
- From time to time, BREAKING changes happen in the `yarn` dependencies. To fix this in a one liner:
    ```bash
    git checkout main && git pull && yarn clean && yarn install && yarn dev
    ```