...
specific light clients for Substrate chains. It uses core components are also part of the 
Substrate building framework and available for every Substrate based project.

Building blockchain apps with Substrate Connect is the same as building any app that uses a special 
RPC node except that nodes don't need to be vetted or trusted.

## What it enables

Substrate Connect provides the necessary infrastructure to run a Substrate light client in any
Javascript or any Node environment, from in-browser applications and extensions, to Electron apps,
