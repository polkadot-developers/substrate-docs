...
[`validate_transaction`](https://paritytech.github.io/substrate/master/sp_transaction_pool/runtime_api/trait.TaggedTransactionQueue.html#method.validate_transaction) is called from the runtime to check for a valid signature and nonce (or output for a UTXO chain) and returns a `Result`.
...
The [`ValidTransaction` struct](https://paritytech.github.io/substrate/master/sp_runtime/transaction_validity/struct.ValidTransaction.html) defines the `requires` and `provides` parameters to build a dependency graph of transactions.
...
[`SignedExtension`](https://paritytech.github.io/substrate/master/sp_runtime/traits/trait.SignedExtension.html) is a trait by which a transaction can be extended with additional data or logic.
...
- [Signed Extension API](https://paritytech.github.io/substrate/master/sp_runtime/traits/trait.SignedExtension.html)
