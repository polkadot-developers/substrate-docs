...
[struct](/rustdocs/latest/sp_runtime/transaction_validity/struct.ValidTransaction.html)
...
For runtimes built with [FRAME](/v3/runtime/frame), `priority` is defined as the `fee` that the transaction
is going to pay. For example:
...
a `Result`. `validate_transaction` checks transactions in isolation, so it will not catch errors like the
same output being spent twice.
...
[`SignedExtension`](/rustdocs/latest/sp_runtime/traits/trait.SignedExtension.html) is a trait
...
execution. This is heavily used in the transaction pool.
...
The `*_unsigned` set of methods can be implemented to encapsulate validation, spam, and replay
