---
title: Session Keys
slug: /v3/docs/concepts/session-keys
version: 3.0 
section: docs
category: concepts
keywords:
...
[Session pallet](/rustdocs/latest/pallet_session), which allows
validators to manage their session keys.

## Use

Session keys are used by validators to sign consensus-related messages. [`SessionKeys`](/rustdocs/latest/sp_session/trait.SessionKeys.html) is a generic,
...
<Message
  type={`gray`}
  title={`Note`}
  text={`This code is just an example of a Substrate node at the
...
[Session pallet](/rustdocs/latest/pallet_session/).
...
<Message
  type={`yellow`}
  title={`Important`}
  text={`For increased security, session keys should be changed every session. This can be done by creating a certificate via signing a session public key
...
- Follow our
  [tutorial to create a local network and generate keys](/v3/tutorials/private-network).
...
- Take a look at
  [`substrate_application_crypto`](/rustdocs/latest/sp_application_crypto),
