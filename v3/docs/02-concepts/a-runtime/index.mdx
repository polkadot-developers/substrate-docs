...
"[state transition function](/v3/docs/getting-started/glossary#state-transition-function-stf)";
it is where Substrate developers define the storage items that are used to represent the
blockchain's [state](/v3/docs/getting-started/glossary#state) as well as the
[functions](/v3/docs/concepts/extrinsics) that allow blockchain users to make
changes to this state.

In order to provide its defining forkless runtime upgrade capabilities, Substrate uses runtimes that
are built as [WebAssembly (Wasm)](/v3/docs/getting-started/glossary#webassembly-wasm)
...
The core Substrate codebase ships with [FRAME](/v3/docs/runtime/frame), Parity's system
...
[pallets](/v3/docs/runtime/pallets). Each pallet encapsulates domain-specific logic that is expressed as a set of a
[storage items](/v3/docs/runtime/storage), [events](/v3/docs/runtime/events),
[errors](/v3/docs/runtime/errors) and
[dispatchable functions](/v3/docs/getting-started/glossary#dispatch). FRAME developers
can [create their own pallets](/v3/docs/runtime/pallets) and reuse existing pallets,
including [over 50 that ship with Substrate](/v3/docs/runtime/frame#prebuilt-pallets).
...
- Learn about the [Substrate FRAME](/v3/docs/runtime/frame).
- Follow a
  [tutorial to develop your first Substrate chain](/v3/tutorials/create-your-first-substrate-chain).
- Follow a [tutorial to add a pallet to your Substrate runtime](/v3/tutorials/add-a-pallet).
