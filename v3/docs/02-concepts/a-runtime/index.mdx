...
"[state transition function](/docs/v3/getting-started/glossary#state-transition-function-stf)";
it is where Substrate developers define the storage items that are used to represent the
blockchain's [state](/docs/v3/getting-started/glossary#state) as well as the
[functions](/docs/v3/concepts/extrinsics) that allow blockchain users to make
changes to this state.

In order to provide its defining forkless runtime upgrade capabilities, Substrate uses runtimes that
are built as [WebAssembly (Wasm)](/docs/v3/getting-started/glossary#webassembly-wasm)
...
The core Substrate codebase ships with [FRAME](/docs/v3/runtime/frame), Parity's system
...
[pallets](/docs/v3/runtime/pallets). Each pallet encapsulates domain-specific logic that is expressed as a set of a
[storage items](/docs/v3/runtime/storage), [events](/docs/v3/runtime/events),
[errors](/docs/v3/runtime/errors) and
[dispatchable functions](/docs/v3/getting-started/glossary#dispatch). FRAME developers
can [create their own pallets](/docs/v3/runtime/pallets) and reuse existing pallets,
including [over 50 that ship with Substrate](/docs/v3/runtime/frame#prebuilt-pallets).
...
- Learn about the [Substrate FRAME](/docs/v3/runtime/frame).
- Follow a
