...
"[state transition function](/v3/reference/glossary#state-transition-function-stf)";
it is where Substrate developers define the storage items that are used to represent the
blockchain's [state](/v3/reference/glossary#state) as well as the
...
are built as [WebAssembly (Wasm)](/v3/reference/glossary#webassembly-wasm)
...
[dispatchable functions](/v3/reference/glossary#dispatch). FRAME developers
