...
- [`BlockBuilder`](/rustdocs/latest/sp_block_builder/trait.BlockBuilder.html)
- [`TaggedTransactionQueue`](/rustdocs/latest/sp_transaction_pool/runtime_api/trait.TaggedTransactionQueue.html)
- [`OffchainWorkerApi`](/rustdocs/latest/sp_offchain/trait.OffchainWorkerApi.html)
- [`AuraApi`](/rustdocs/latest/sp_consensus_aura/trait.AuraApi.html)
- [`SessionKeys`](/rustdocs/latest/sp_session/trait.SessionKeys.html)
- [`GrandpaApi`](/rustdocs/latest/sp_finality_grandpa/trait.GrandpaApi.html)
- [`AccountNonceApi`](/rustdocs/latest/frame_system_rpc_runtime_api/trait.AccountNonceApi.html)
- [`TransactionPaymentApi`](/rustdocs/latest/pallet_transaction_payment_rpc_runtime_api/trait.TransactionPaymentApi.html)
- [`Benchmark`](/rustdocs/latest/frame_benchmarking/trait.Benchmark.html)
...
layers of Substrate. But there are a few data types need to be defined and must fulfill a particular
...
defines additional runtime primitives and provides a framework that makes it easy to construct a
runtime by composing modules, called [pallets](/v3/runtime/pallets). Each pallet encapsulates
domain-specific logic that is expressed as a set of a [storage items](/v3/runtime/storage),
[events](/v3/runtime/events), [errors](/v3/runtime/errors) and
[dispatchable functions](/v3/getting-started/glossary#dispatch). FRAME developers
can [create their own pallets](/v3/runtime/pallets) and reuse existing pallets,
