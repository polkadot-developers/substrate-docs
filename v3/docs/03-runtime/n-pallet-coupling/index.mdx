...
Unlike tight coupling, in loosely coupling pallets we just specify traits and function interfaces that
...
`Currency<Self::AccountId>` so we can tap into the balance transfer logic by calling
...
better practice from a system design perspective. It guarantees better maintainability, reusability,
