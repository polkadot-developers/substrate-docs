...
that _should_ be a
[`DispatchError`](/rustdocs/latest/frame_support/dispatch/enum.DispatchError.html)
...
- the [`#[pallet::error]` macro](macros#palleterror) (FRAME v2).
...
<Message
  type={`gray`}
  title={`Note`}
  text={`In FRAME v1, in order to emit custom errors from a pallet, the pallet must
  configure the \`Error\` type in \`decl_module!\`. See the [Rust docs](/rustdocs/latest/frame_support/macro.decl_error.html#usage) for more details.`}
/>
...
- [`decl_error!` macro](/rustdocs/latest/frame_support/macro.decl_error.html)
