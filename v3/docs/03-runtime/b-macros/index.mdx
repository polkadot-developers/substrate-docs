...
This requires all storage to implement the trait `traits::StorageInfoTrait`, thus all keys and value
types must bound `pallet_prelude::MaxEncodedLen`. Some individual storage can opt-out from this
constraint by using `#[pallet::unbounded]`, see `#[pallet::storage]` documentation.
