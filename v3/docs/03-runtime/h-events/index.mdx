...
Substrate provides a default implementation of how to deposit an event using macros, which varies slightly depending on [FRAME version adopted](../macros#overview-of-frame-versioning). In FRAME v1:
...
like the [Polkadot-JS API](/docs/v3/toolchains/polkadot-js), however you can implement your own
`deposit_event` function if you want to handle events differently.

## Supported Types

Events can emit any type which supports the [Parity SCALE codec](../../advanced/scale-codec).
...
System module. Otherwise, the [Polkadot-JS API](/docs/v3/toolchains/polkadot-js) supports a WebSocket
...
- Learn more about using the [Polkadot-JS API](/docs/v3/toolchains/polkadot-js).
...
- [`decl_event!` macro](/rustdocs/latest/frame_support/macro.decl_event.html)
- [`decl_module!` macro](/rustdocs/latest/frame_support/macro.decl_module.html)
