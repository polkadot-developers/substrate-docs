...
[`frame-metadata`](https://substrate.dev/rustdocs/latest/frame_metadata) and
[`parity-scale-codec`](https://substrate.dev/rustdocs/latest/parity_scale_codec) libraries.
...
Substrate nodes expose [a JSON-RPC API](https://substrate.dev/rustdocs/latest/sc_rpc) that you can
...
Every storage item that is defined in a pallet will have a corresponding metadata entry. Metadata entries like these are generated from [macros](macros) using associated types from the [`frame-system`](https://substrate.dev/rustdocs/latest/frame_system/pallet/trait.Config.html) crate. For example:
...
- [Metadata](https://substrate.dev/rustdocs/latest/frame_metadata)
