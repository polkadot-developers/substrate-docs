...
[extrinsic calls](../../concepts/extrinsics), [events](../events), constants, and errors that are
...
[`frame-metadata`](/rustdocs/latest/frame_metadata) and
[`parity-scale-codec`](/rustdocs/latest/parity_scale_codec) libraries.
...
Substrate nodes expose [a JSON-RPC API](/rustdocs/latest/sc_rpc) that you can
...
The `result` field contains the blockchain metadata as a [SCALE-encoded](../../advanced/scale-codec)
...
especially when considering [signed extrinsics](../../concepts/extrinsics).
...
Every storage item that is defined in a pallet will have a corresponding metadata entry. Metadata entries like these are generated from [macros](../macros) using associated types from the [`frame-system`](/rustdocs/latest/frame_system/pallet/trait.Config.html) crate. For example:
...
- [Metadata](/rustdocs/latest/frame_metadata)
