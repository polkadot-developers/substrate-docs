...
[the metadata of each pallet](https://paritytech.github.io/substrate/master/frame_metadata/index.html),
...
Metadata entries like these are generated from [macros](/v3/runtime/macros) using associated types from the [`frame-system`](https://paritytech.github.io/substrate/master/frame_system/pallet/trait.Config.html) crate. For example:
...
Storage metadata provides blockchain clients with the information that is required to query [the JSON-RPC's storage function](https://paritytech.github.io/substrate/master/sc_rpc/state/struct.StateClient.html#method.storage) to get information for a specific storage item.
