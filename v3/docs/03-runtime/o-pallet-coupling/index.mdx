...
tightly couple, you explicitly specify the pallet `Config` trait to be bound by the `Config` trait
...
certain types need to be bound of, and doesn't care how they are implemented **inside the pallet**.

The actual implementation happens **outside of the pallet** during our runtime configuration
(usually happening in the file `/runtime/src/lib.rs`). Here one may choose to configure it with
another pallet that have implemented these traits, or declare a totally new struct,
...
[Currency trait](https://substrate.dev/rustdocs/latest/frame_support/traits/tokens/currency/trait.Currency.html)
in this way. 
