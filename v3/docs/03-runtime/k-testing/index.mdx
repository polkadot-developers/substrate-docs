...
[`substrate_state_machine`](/rustdocs/latest/sp_state_machine) referred to as
...
`TestExternalities`, and setting the block number to be 1.
...
Custom implementations of [Externalities](/rustdocs/latest/sp_externalities) allow developers
to construct runtime environments that provide access to features of the outer node. Another example
of this can be found in [`offchain`](/rustdocs/latest/sp_core/offchain), which maintains its own
