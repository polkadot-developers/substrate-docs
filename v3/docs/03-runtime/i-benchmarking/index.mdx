...
one block. This is further explained in the [transaction weight section](/v3/concepts/weight).
...
4. Once you have written your benchmarks, you should make sure they execute properly by testing
   ...
   The `impl_benchmark_test_suite!` macro takes three inputs: the Module struct generated by your pallet,
...
2. Add your new pallet to your runtime just as you would any other pallet. If you need more details
   check out our tutorial to [Add a Pallet to Your Runtime Tutorial](/tutorials/v3/add-a-pallet).
...
Once this is done, you should be able to run the `benchmark` subcommand from your project's top level directory:
...
- `steps` and `repeat` means how many steps will be taken to walk through each variable range, and
  how many times the execution state will be repeated.
...
Here, associated function `set_dummy` take a parameter. This is because we have the line `let b in 1 .. 1000;` in our `set_dummy` benchmarking function, indicating the execution time likely be affected
by this parameter.
...
Try to keep an extrinsic simple and to perform only one function. Sometimes it maybe better to separate
...
1. <span id="footnote-ref-hardware">The</span> reference hardware use for benchmarking
   Substrate is an Intel Core i7-7700K CPU with 64GB of RAM and an NVMe SSD.
