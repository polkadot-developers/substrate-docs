...
- `transaction_version`: The version of the extrinsics interface, useful to synchronize firmware updates for hardware wallets and to sign transactions correctly. 
  ...
  If this number is updated, then the `spec_version` must also be updated which can be automated in your chain's CI process. 

- `apis` is a list of supported
  [runtime APIs](/rustdocs/latest/sp_api/macro.impl_runtime_apis.html) along
  with their versions.
