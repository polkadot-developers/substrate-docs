...
<AccentButton text={`Start the ink! workshop`} link={`/tutorials/v3/ink-workshop/pt1`} />
...
contract code and some persistent contract storage. A noteable beaviour that arises from this
is that a contract's account can receive balance without having its code executed by submitting
a plain transfer transaction.
...
<AccentButton text={`Start the Frontier workshop`} link={`/tutorials/v3/frontier`} />
