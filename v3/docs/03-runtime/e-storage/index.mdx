...
FRAME's [`Storage` module](/rustdocs/latest/frame_support/storage/index.html) gives runtime developers access to Substrate's flexible storage APIs, which can support any value that is encodable
by [Parity's SCALE codec](/v3/advanced/scale-codec/index.html). These include:
