...
FRAME's [`Storage pallet`](/rustdocs/latest/frame_support/storage) gives runtime developers access to Substrate's flexible storage APIs, which can support any value that is encodable
by [Parity's SCALE codec](../../advanced/scale-codec). These include:
...
initializing storage, all of which have entry points in the `#[pallet::genesis_config]` macro. These mechanisms
...
Storage items that are tagged for genesis configuration, such as [`Struct pallet_balances::GenesisConfig`](/rustdocs/latest/pallet_balances/pallet/struct.GenesisConfig.html)) for example, will have a corresponding attribute on this
data type.
...
the [`BuildStorage` trait](/rustdocs/latest/sp_runtime/trait.BuildStorage.html). For example, in [`Struct node_template_runtime::GenesisConfig`](/rustdocs/latest/node_template_runtime/struct.GenesisConfig.html),
...
The `#[pallet::genesis_config]` provides an extension that will add an attribute to the pallet's 
