...
by [Parity's SCALE codec](/v3/advanced/scale-codec). These include:

- [Storage Value](/rustdocs/latest/frame_support/storage/trait.StorageValue.html) - used to store any single value, such as a `u64`.
- [Storage Map](/rustdocs/latest/frame_support/storage/trait.StorageMap.html) - used to store a key-value hash map, such as a balance-to-account mapping.
- [Storage Double Map](/rustdocs/latest/frame_support/storage/trait.StorageDoubleMap.html) - used as an implementation of a storage map with two keys to provide the ability to efficiently removing
  all entries that have a common first key.
- [Storage N Map](/rustdocs/latest/frame_support/storage/trait.StorageNMap.html) - used to store a hash map with any arbitrary number of keys, it can be used as a basis to build a Triple Storage Map, a Quadruple Storage Map and so on.
...
Runtime storage items are created in the [`#[pallet::storage]` macro](/rustdocs/latest/frame_support/attr.pallet.html#storage-palletstorage-optional) 
in any FRAME-based pallet. Here is an example of declaring the 3 different types of storage items:
...
`pub(super)` keyword. Blockchain storage is always publicly
...
initializing storage, all of which have entry points in the `#[pallet::genesis_config]` macro. This mechanism
results in the creation of a `GenesisConfig` data type that implements
the [`BuildModuleGenesisStorage` trait](/rustdocs/latest/sp_runtime/trait.BuildModuleGenesisStorage.html)
and will be added to the pallet that contains the storage items.
Storage items that are tagged for genesis configuration, such as 
[`pallet_balances::GenesisConfig`](/rustdocs/latest/pallet_balances/pallet/struct.GenesisConfig.html) 
for example, will have a corresponding attribute on this data type.
...
extension allows you to define how `genesis_configuration` is built within the pallet itself (this gives you access to the pallet's private
functions). Like `genesis_config`, the `genesis_build` extension accepts a single parameter, but in this case the
parameter is always required and must be a closure, which is essentially a function. The `genesis_build`
closure will be invoked with a single parameter whose type will be the pallet's `GenesisConfig` type. 
This gives you easy access to all the attributes of the `GenesisConfig` type.

The `genesis_build` extension must be used alongside the `genesis_config` extension. A
pallet's `GenesisConfig` type will have an attribute that corresponds to what was set in `genesis_config` and
whose value will be set in the chain specification, but it will be the value returned by the `genesis_build`
closure that will be used to set the storage item's genesis value.

Here is an example that demonstrates using `genesis_build` to set the initial value of a storage item. In
...
- Visit the reference docs for
  [StorageValue](/rustdocs/latest/frame_support/storage/trait.StorageValue.html),
  [StorageMap](/rustdocs/latest/frame_support/storage/trait.StorageMap.html),
  [StorageDoubleMap](/rustdocs/latest/frame_support/storage/trait.StorageDoubleMap.html)
  and StorageNMap to learn more about their APIs.
