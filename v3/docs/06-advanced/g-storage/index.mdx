...
All of Substrate's [higher-lever storage abstractions](/docs/v3/runtime/storage) are built on top of this
...
[Substrate's runtime storage APIs](/docs/v3/runtime/storage) expose a number of storage item types; keep
reading to learn how to calculate storage keys for the different types of storage items.

### Storage Value Keys

To calculate the key for a simple [Storage Value](/docs/v3/runtime/storage#storage-value), take the
...
[non-cryptographic](/docs/v3/runtime/storage#cryptographic-hashing-algorithms) TwoX 128 hash algorithm is
...
Like Storage Values, the keys for [Storage Maps](/docs/v3/runtime/storage#storage-map) are equal to the
...
[hashing algorithm](/docs/v3/runtime/storage#hashing-algorithms) (the one that was declared in
[the `decl_storage` macro](/docs/v3/runtime/storage#declaring-storage-items)) when determining the hashed
...
is using
[the transparent Blake2 128 Concat hashing algorithm](/docs/v3/runtime/storage#transparent-hashing-algorithms):
...
[a transparent hashing algorithm](/docs/v3/runtime/storage#transparent-hashing-algorithms). Although the
...
- Learn how to add [storage items](/docs/v3/runtime/storage) into your Substrate runtime modules.
