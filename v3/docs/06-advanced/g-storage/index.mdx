...
All of Substrate's [higher-lever storage abstractions](/v3/docs/runtime/storage) are built on top of this
...
[`state_db` layer](/rustdocs/latest/sc_state_db) that maintains the
...
[Substrate's runtime storage APIs](/v3/docs/runtime/storage) expose a number of storage item types; keep
reading to learn how to calculate storage keys for the different types of storage items.

### Storage Value Keys

To calculate the key for a simple [Storage Value](/v3/docs/runtime/storage#storage-value), take the
...
[non-cryptographic](/v3/docs/runtime/storage#cryptographic-hashing-algorithms) TwoX 128 hash algorithm is
...
Like Storage Values, the keys for [Storage Maps](/v3/docs/runtime/storage#storage-maps) are equal to the
...
[hashing algorithm](/v3/docs/runtime/storage#hashing-algorithms) (the one that was declared in
[the `decl_storage` macro](/v3/docs/runtime/storage#declaring-storage-items)) when determining the hashed
...
is using
[the transparent Blake2 128 Concat hashing algorithm](/v3/docs/runtime/storage#transparent-hashing-algorithms):
...
[a transparent hashing algorithm](/v3/docs/runtime/storage#transparent-hashing-algorithms). Although the
...
Substrate's [FRAME Support crate](/rustdocs/latest/frame_support)
...
- Learn how to add [storage items](/v3/docs/runtime/storage) into your Substrate runtime modules.
