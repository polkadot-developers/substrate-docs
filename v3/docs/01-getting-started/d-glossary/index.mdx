---
title: Glossary
slug: /docs/v3/getting-started/glossary
version: 3.0 
section: docs
...
[FRAME Elections Phragmen pallet](../../runtime/frame#elections-phragmén) that materializes as a
...
- [Transaction](#transaction) authentication: in [runtimes](#runtime) where transactions and
  [accounts](../../concepts/account-abstractions) are used, accounts are identified and
...
[well-known development accounts](/docs/v3/toolchains/subkey#well-known-keys) (Alice, Bob, Charlie, Dave,
Ferdie, and Eve) are generated from the same dev phrase. The dev phrase is:
`bottom drive obey lake curtain smoke basket hold race lonely fit walk`.

Many tools in the Substrate ecosystem, such as [`subkey`](/docs/v3/toolchains/subkey), allow users to
...
There are two [executor](../../advanced/executor) implementations present in
...
[signed](../../concepts/extrinsics#signed-transactions) or
[unsigned](../../concepts/extrinsics#unsigned-transactions), and
[inherents](../../concepts/extrinsics#inherents), which are inserted by
[block authors](#author).

## Existential Deposit

Within the [Balances pallet](/docs/v3/runtime/frame#balances), this is the minimum
...
[chain specification](/docs/v3/toolchains/chain-specs) file, which makes it easy to use a
...
expose [lots of helpful metadata](/docs/v3/runtime/metadata).
...
[three built-in origins](/docs/v3/runtime/origins#raw-origins); [pallet](#pallet)
...
[a simple key/value map](/docs/v3/advanced/storage) where both keys and values can be
...
[runtime storage](/docs/v3/runtime/storage).
...
[signatures](/docs/v3/concepts/extrinsics#signed-transactions) or
[signed extensions](/docs/v3/concepts/extrinsics#signed-extension).
