...
The Substrate [client](/v3/reference/glossary#node) is an application that runs a Substrate-based blockchain node.
...
- **Runtime**: the logic that defines how blocks are processed, including state transition logic. In Substrate, runtime code is
  compiled to [Wasm](/v3/reference/glossary#webassembly-wasm) and becomes part of the blockchain's
