...
relevantSkills: 
  ...
  - React
---
<Message type='gray' title='Note' text={`If you haven't completed [Part I](/tutorials/v3/kitties/pt2) and just want to focus on building the Kitties front-end, 
...
1. Proceed by pasting in the following code snippet:
  ...
  We'll use `api.query.kitties.kittyCnt` to listen for a change in the amount of Kitties,
...
1. Paste in the follow snippet to extract the last 64 bytes of the storage key:
...
1. Use that value in the subscription function to transform an entry to its corresponding hash:
...
1. Now all that's left to do for our component to listen for changes in our node's runtime storgae is
