...
You'll notice the following structure (we're only including the directories we care about for this tutorial):
...
Before moving on to the next section, we recommend you read a little Polkadot JS API documentation to understand the basics of how we will be querying storage and triggering transactions.
...
1.  Similarly for `subscribeKitties`, paste the following code snippet:
    ...
    `kitties` is a [storage map](https://paritytech.github.io/substrate/master/frame_support/storage/types/struct.StorageMap.html), Substrate supports returning the whole map of objects and we use Polkadot-JS API to iterate through the map.
...
1. Write out the contents for the `Card` component.
   ...
   Before closing the `<Card/>` component we want to render the `TransferModal` and `SetPrice` components we previously built &mdash; **only if the Kitty is transferable by its owner**. Paste this code snippet to handle this functionality:
