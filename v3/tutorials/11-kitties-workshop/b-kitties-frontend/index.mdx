...
[Substrate Frontend Template][substrate-frontend-template] components use Polkadot-JS API and
...
1.  Now all that's left to do for our component is to listen for changes in our node's runtime storage is to pass in `subscribeKittyCnt` and `subscribeKitties` to React's `useEffect` function.
