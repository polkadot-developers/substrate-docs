...
Next, we increment the `CountForKitty` using the storage getter function `Self::count_for_kitty()`.
...
- **\`<CountForKitty<T>>\`**: A count of all Kitties in existence.
`}
/>
...
Notice that we've already used `CountForKittyOverflow` and `ExceedMaxKittyOwned` in our `mint` function.
