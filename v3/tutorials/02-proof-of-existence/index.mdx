...
Proof of existence is an approach to validating the authenticity and ownership of a digital object by using the information stored on the blockchain about the object.
Because the blockchain associates a timestamp and signature with the object, the blockchain record can be used to verify—to prove—that a particular object existed at a specific date and time and the owner of record at that date and time.
...
The hash enables the blockchain to store files of arbitrary size efficiently using a unique hash value. 
...
Because the account information is stored as part of the transaction, the controller of the account can later prove that he or she was the person who initially uploaded the file.
...
* You have installed Rust and the Rust toolchain.
...
Therefore, the first step is to remove some files and content from the file in the node template directory.

1. Open a terminal shell and navigate to the root directory for the node template.

1. Change to the `pallets/template/src` directory byt running the following command:
...
The `sp-std` crate provides many standard Rust library functions amodified to be compatible with `no_std` configuration. 
To use the  `sp-std` crate, you _must_ update the pallet dependencies in the `Cargo.toml` file.
...
To address this requirement, you can create a [hash map](https://en.wikipedia.org/wiki/Hash_table) that maps each proof to its owner and records the block number the proof was made. 
...
In this tutorial, you learned how to create a new custom pallet, how to integrate your pallet and run a customized blockchain node.
