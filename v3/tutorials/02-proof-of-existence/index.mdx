...
Events can contain some additional data. In this case, each event will also display who trigger the
event (`AccountId`), and the proof data (as `Vec<u8>`) that is being stored or removed. Note that
convention is to include an array with descriptive names for these parameters at the end of event
documentation.
...
A new tab should open in your web browser and you'll the following interface.
