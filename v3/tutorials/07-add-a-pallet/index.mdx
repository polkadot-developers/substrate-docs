...
The first thing you need to do is to import the `pallet-nicks` crate in your
...
In the code snippet above, you will notice that we set
`default-features = false`. If you TK>>>>>>>> the `Cargo.toml`, you will find something like:
...
- Module **Event**s: Because it uses the `#[pallet::events]` macro. You will notice that in the case of
