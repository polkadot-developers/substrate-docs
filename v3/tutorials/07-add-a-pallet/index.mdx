---
title: Add FRAME's Nicks Pallet to Runtime
...
similar pattern to add additional FRAME pallets to your runtime. However you should note that each
...
<Message
  ...
  text={`If you run into issues on this tutorial, **we are here to help!** You can
...
You should already have the [latest version of the Node Template](https://github.com/substrate-developer-hub/substrate-node-template/tree/latest) compiled on your computer from when you completed the
...
Open `substrate-node-template/runtime/Cargo.toml` and you will see all the dependencies
...
This is telling Cargo to find the crate from git repository [`paritytech/substrate`](https://github.com/paritytech/substrate) with commit
...
use the `Balance` type, a type alias of `u128`, to track balances. If you were developing a chain
where it was important to optimize storage, you could use any unsigned integer type that was at
...
correctly. You can also view a [full solution including the `Nick` pallet](https://github.com/substrate-developer-hub/substrate-node-template/tree/tutorials/solutions%2Fadd-a-pallet-v3).
...
**crates.io** allows permissionless publishing of any Rust modules. You could learn the procedure by
