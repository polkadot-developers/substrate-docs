---
title: Add the Contracts pallet to the runtime
slug: /tutorials/v3/add-contracts-pallet
version: 3.0 
hideNav: true
section: tutorials
...
<FastTrackPlayground
  ...
  playgroundLink={`/playground?deploy=node-template#config`}
/>
...
The node template includes a [runtime](/v3/getting-started/glossary#runtime) that is constructed using the [FRAME](/v3/runtime/frame) development framework and includes some **pallets** to get you started. 

In this tutorial, you'll learn how to add the [Contract pallet](/rustdocs/latest/pallet_contracts/index.html) to the node template. 
...
This tutorial illustrates how to configure the traits that the Contracts pallet requires.
...
<TutorialObjective
  data={{
    textLineOne:
      'Add the Contracts pallet to your node template',
...
<TutorialObjective
  data={{
    textLineOne:
      'Implement the Contracts pallet configuration trait.',
...
<TutorialObjective
  data={{
    textLineOne:
      'Learn how to publish a custom pallet to Github or Crates.io ',
    url: '/tutorials/v3/add-a-pallet#publish-your-own-pallet',
  }}
/>


## Add the Contracts pallet to the node template

The Contracts pallet allows blockchain users to pay a deposit to reserve a nickname and associate it with an account they control.
...
1. Import the `pallet-nicks` crate to make it available to the node template runtime by adding it to the list of dependencies.
  ...
  It is important to note that the Substrate runtime compiles to both a native Rust [`std`](https://doc.rust-lang.org/std/) binary and a [WebAssembly (wasm)](https://webassembly.org/) binary. For more informtion about compiling [`std`](https://doc.rust-lang.org/std/) and [`no_std`](https://rust-embedded.github.io/book/intro/no-std.html) features, see XXX.
...
To see what you need to implement for the Nicks pallet specifically, see the [`pallet_nicks::Config` documentation](/rustdocs/latest/pallet_nicks/trait.Config.html) or the definition of the trait itself in
[the source code](https://github.com/paritytech/substrate/blob/master/frame/nicks/src/lib.rs) of the Nicks pallet. 
...
1. Identify the types that the Nicks pallet exposes.
    ...
    * Module **Event**s Because it uses the `decl_event!` macro. In the Nicks pallet, the `Event` keyword is parameterized with respect to a type, `T` because at least one of the events defined by the Nicks pallet depends on a type that is configured with the `Config` configuration trait.
...
1. Select the [`setName` dispatchable](/rustdocs/latest/pallet_nicks/enum.Call.html#variant.set_name) as the function to call from the `nicks` pallet. 
...
1. Observe the status of the call and the [events](/rustdocs/latest/pallet_nicks/enum.RawEvent.html) emitted by the Nicks
pallet.
...
- For a bare FRAME pallet with detailed comments about what you can access within FRAME, see [this example in `substrate`](https://github.com/paritytech/substrate/tree/master/frame/example).
