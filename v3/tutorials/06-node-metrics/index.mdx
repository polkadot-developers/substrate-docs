...
- Install OSS Grafana [here](https://grafana.com/grafana/download?edition=oss).
...
Now startup Grafana, and you will log in and navigate to it in a browser (**the default is http://localhost:3000/**).
Log in (with the default user `admin` and password `admin`) and navigate to the data sources page at
...
<Message
  type={`green`}
  title={`Tip`}
  text={`The Prometheus port Grafana needs is NOT the one you set in the \`prometheus.yml\` file (http://localhost:9615) for where your node is publishing it's data.`}
/>
...
- See the [docs for Prometheus in Substrate](https://paritytech.github.io/substrate/master/prometheus/index.html).
