---
title: Sudo Upgrade
slug: /tutorials/v3/forkless-upgrades/sudo-upgrade
version: 3.0 
section: tutorials
...
runtime upgrade using the [Sudo pallet](/docs/v3/runtime/frame#sudo).
...
By default, the [well-known Alice account](/docs/v3/toolchains/subkey#well-known-keys) is
...
[weight](/docs/v3/concepts/weight), which is used for resource accounting. FRAME's
...
<Message
  ...
  text=
	...
	use [transaction priority](/docs/v3/concepts/tx-pool#transaction-priority)
...
[the `Operational` class](/docs/v3/runtime/weights-and-fees#operational-dispatches) of dispatchable
...
As the name of the [Sudo pallet](/docs/v3/runtime/frame#sudo) implies, it provides
...
[storage item](/docs/v3/runtime/storage): the ID of the account that has access to the
pallet's [dispatchable functions](/docs/v3/getting-started/glossary#dispatch). The Sudo
...
- `authoring_version`: The authorship version for
  [block authors](/docs/v3/getting-started/glossary#author).
...
- `transaction_version`: The version of the
  [dispatchable function](/docs/v3/getting-started/glossary#dispatch) interface.
