---
title: Sudo Upgrade
slug: /v3/tutorials/forkless-upgrades/sudo-upgrade
version: 3.0 
section: tutorials
...
runtime upgrade using the [Sudo pallet](/v3/docs/runtime/frame#sudo).

## Steps
### 1. Start the Template Node
...
By default, the [well-known Alice account](/v3/docs/toolchains/subkey#well-known-keys) is
...
[weight](/v3/docs/concepts/weight), which is used for resource accounting. FRAME's
...
<Message
  ...
  text=
	...
	use [transaction priority](/v3/docs/concepts/tx-pool#transaction-priority)
...
The [`set_code` function](/rustdocs/latest/src/frame_system/lib.rs.html#329-337)'s
weight annotation also specifies that the extrinsic call is in
[the `Operational` class](/v3/docs/runtime/weights-and-fees#operational-dispatches) of dispatchable
...
### 2. Use `sudo` to dispatch

As the name of the [Sudo pallet](/v3/docs/runtime/frame#sudo) implies, it provides
...
[storage item](/v3/docs/runtime/storage): the ID of the account that has access to the
pallet's [dispatchable functions](/v3/docs/getting-started/glossary#dispatch). The Sudo
...
- `authoring_version`: The authorship version for
  [block authors](/v3/docs/getting-started/glossary#author).
...
- `transaction_version`: The version of the
  [dispatchable function](/v3/docs/getting-started/glossary#dispatch) interface.
