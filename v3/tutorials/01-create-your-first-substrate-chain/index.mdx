...
- Data handling capabilities for external—[extrinsic]()—information
- [Runtime environment]()
...
![Blockchain project repository forks](../../../../src/images/tutorials/01-create-your-first-chain/forks.png)
...
The tutorial assumes you have no prior experience with or knowledge of Substrate. 
...
For this tutorial, you download and use working code. 
...
Substrate development is easiest on UNIX-based operating systems like macOS or Linux. If you are using Microsoft Windows, see [XX]().
...
1. Locate your operating system in the following table and run the appropriate commands for your development environment.
  ...
  | For this operating system | Run the following commands
  | ------------------------- | --------------------------
...
The previous steps walked you through the installation and configuration of Rust and the Rust toolchain. 
It is also possible to automate the steps using a script.
If you want to try installing and configuring Rust using a script, open a terminal shell on your computer and run the following command:
...
1. Clone the node template repository using the version `v3.0.0+monthly-2021-05` branch by running the following command:
...
You can use this front-end template as a starting point for creating user interfaces for your own projects in the future.

The front-end template requires [Yarn](https://yarnpkg.com/) and [Node.js](https://nodejs.org/). If you don't have these tools, you must install them first.
...
1. Open http://localhost:8000/ in a browser to view the front-end template.
    ...
    ![Front-end template top section](../../../images/tutorials/01-create-your-first-chain/blockchain-top.png)
...
1. In the Balances table, notice the predefined accounts—such as dave—that have no funds associated with them.
    ...
    Under the Balances table, the front-end template displays a **Transfer** component. 
...
1. Notice that the Balances table is updated with the transfer.
...
1. Check the Events component to see events related to the transfer you just completed.
    ...
    ![image-name](../../../../src/images/docs/getting-started/technical-freedom.png)
...
Stopping the local node stops the blockchain from progressing and resets the node template to its initial state so that you can return to it for use in this tutorial or other tutorials.
...
- How to view and interact with the blockchain node using a front-end user interface template.
...
You can explore these components on your own or through subsequent tutorials that guide you through what each component does.

For a guided tour of the front-end template components, see the following tutorials:

- [Explore blockchain metadata]().
- [Call extrinsic functions]().
- [Add a new pallet]().
- [Upgrade the runtime]().

If you've experienced any issues with this tutorial or want to provide feedback:

- Ask a question on Stack Overflow using the substrate tag.

- Contact Parity on Element.
