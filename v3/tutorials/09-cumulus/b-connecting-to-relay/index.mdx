...
It is called "para ID", because the same ID can be seen as its parachain ID or
parathread ID, depending on your current relay chain slot state. To reserve one, you need
to have sufficient amount of the currency on the network to reserve the ID. Check for the amount on
your target relay chain.

The relay chain is responsible for allotting all para IDs and will be simply the increment starting
at `2000` for all chains connecting that are not system
or <ExternalLink url="https://wiki.polkadot.network/docs/learn-common-goods"> common good parachains</ExternalLink>
that use a different method to allocate the para ID.

The easiest way to reserve your para ID is via
the <ExternalLink url="https://polkadot.js.org/apps/?rpc=ws%3A%2F%2F127.0.0.1%3A9944#/parachains/parathreads">Polkadot-JS Apps</ExternalLink>
...
We'll begin by generating a few files based on the parachain template with **a known reserved para**
**ID that we control** (para ID `2000`). You should be able to use whatever para ID and parachain
collator binary you have roughly following these instructions.
...
connect a parachian with any previous state to a relay chain (outside of the genesis state you
generated above). All parachains must start from block 0 on the relay chain. Eventually, migrating a
solochain built on substrate is expected to be possible, but this functionality is not planned
anytime soon. 
`}
/>
