...
with <ExternalLink url="https://wiki.polkadot.network/docs/en/learn-auction">parachain auctions</ExternalLink>
on Polkadot and Kusama, but for this tutorial we will do it with `sudo` call.
...
are also finalized. This is how Polkadot achieves _pooled, common_ security for it's parachains!
...
parachains and interact with them. We are finally ready to submit extrinsics on your parachain!
...
A parachain is it's own chain, but one key feature of connecting to a _common_ relay chain is the
ability to communicate _between_ the connected chains. This area of functionality at the cutting
edge development, and for now is not implemented in this workshop.
...
If you _must_ purge your chain database while for whatever reason, you need to deregister and
re-register it. It may be easier in testing to instead just purge all the chains. To purge the
nodes, run:
...
It is also possible to start non-collating full node for a parachain. In this case, simply leave out
the `--collator` flag.
