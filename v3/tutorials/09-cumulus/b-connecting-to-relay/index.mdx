...
<Message
  ...
  title={`Information`}
  text={`
    You will need to connect to a relay chain to submit this extrinsic!
  In testnets, your Parachain ID will be 2000 for your first parathread reservation.
  `}
/>
...
To register a parachain, the relay chain needs to know the parachain's [genesis state](/v3/runtime/chain-specs#the-genesis-state). The collator node can export that state to a file.
...
The Wasm blob does not depend on the para ID, so we are not providing that flag.
If you are launching multiple parachains using the exact same runtime, you do not need to regenerate the Wasm blob each time.
...
<Message
  ...
  text={`
    ...
    All parachains must start from block 0 on the relay chain. Eventually, migrating a solo chain built on Substrate is expected to be possible, but this functionality is not planned anytime soon.
    \n
