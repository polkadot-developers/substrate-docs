...
[Cumulus](https://github.com/paritytech/cumulus) to create your own parachain, connect it to the
relay chain, all in a local testnet environment.

Please learn about parachains first **before** continuing the tutorial:
...
If you are here _without_ any former Substrate experience, you are likely not understand or complete
this tutorial. We assume you have completed these tutorials:
...
error. This generally happens on building the \`polkadot-service\` crate so be sure to monitor your
RAM usage with tools like [htop](https://htop.dev/) and look out as swap memory starting to be
used.
`}
/>
...
longer build time for more limited memory usage.
...
connecting to. To have the least hiccups, be sure you are using the corresponding tagged
version of Polkadot and Parachain Template when working on this tutorial. So if you are using [Polkadot \`v0.9.10\`](https://github.com/paritytech/polkadot/tree/v0.9.10),
be sure to us the equivalent version of the [Parachain Template \`polkadot-v0.9.10\`](https://github.com/substrate-developer-hub/substrate-parachain-template/tree/polkadot-v0.9.10).
  `}
/>
...
> After you start the compilation, it will take a while (30 mins to 60 mins) to complete. So please
> continue to read through the rest of the workshop.
...
> Again, it will take a while (30 mins to 60 mins) to complete.
...
the outset. Looking at template runtime `Cargo.toml`, you will spot a few unfamiliar pallets.

#### Parachain info pallet

Parachain Template runtime has integrated
[`parachain-info` pallet](https://paritytech.github.io/cumulus/parachain_info/pallet/index.html) in.
...
[as shown here](https://github.com/substrate-developer-hub/substrate-parachain-template/blob/latest/runtime/src/lib.rs#L648-L652):
...
- [rococo-custom-2-raw.json](/assets/tutorials/cumulus/chainspecs/rococo-custom-2-raw.json):
  A two-validator relay chain with Alice and Bob as authorities. Useful for registering a single
  parachain.

- [rococo-custom-3-raw.json](/assets/tutorials/cumulus/chainspecs/rococo-custom-3-raw.json):
  A three-validator relay chain identical to `rococo-custom-2-raw` spec, with Charlie as the third
  validator.

- [rococo-custom-4-raw.json](/assets/tutorials/cumulus/chainspecs/rococo-custom-4-raw.json):
...
values. Once this node is launched, no other nodes on the relay chain or parachains can
use these ports.
...
Finally he has added a `--bootnodes` flag. This flag is not strictly necessary if you are
...
<AccentButton
  text={`Don't need a custom relay chain spec? Jump to connecting to your relay chain ➔`}
  link={`../pt2`}
/>
...
- [rococo-custom-2-plain.json](/assets/tutorials/cumulus/chainspecs/rococo-custom-2-plain.json)
- [rococo-custom-3-plain.json](/assets/tutorials/cumulus/chainspecs/rococo-custom-3-plain.json)
- [rococo-custom-4-plain.json](/assets/tutorials/cumulus/chainspecs/rococo-custom-4-plain.json)
