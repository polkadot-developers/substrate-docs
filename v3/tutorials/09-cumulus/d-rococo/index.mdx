...
Lastly, you MUST have you collator's peering ports for hte embedded relay chain and your parachain publicly accessible and discoverable.
...
## Use `sudo` to register a slot

Here is the flow that Parity uses on the back end, and you can use for your own Rococo-like relay chains:

The following parameters need to be to configured in the relay chain runtime, as an example:
```rust
pub const LeasePeriod: BlockNumber = 7 * DAYS;
// The number of lease periods a permanent parachain slot lasts
// (multiplied by LeasePeriod to get the slot duration in blocks)
...
pub const MaxTemporarySlotPerLeasePeriod: u32 = 10;
```

#### Temporary slots

Initialize a para with `parasSudoWrapper.sudoScheduleParaInitialize` and wait for it to be onboarded as a parathread.
Then assign a temporary parachain slot with `parasSudoWrapper.sudoAssignTempParachainSlot`, specifying the para Id and lease period start (Current lease period or next one).

![sudoAssignTempParachainSlot.png](../../../../src/images/tutorials/09-cumulus/sudoAssignTempParachainSlot.png)
...
`parasSudoWrapper` pallet keeps track of temporary slots so it can distribute turns fairly between temp slots.

![temporarySlots.png](../../../../src/images/tutorials/09-cumulus/temporarySlots.png)

#### Permanent slots

You can assign a permanent slot with `parasSudoWrapper.sudoAssignPermParachainSlot` or unassign a slot (permanent or temporary) with `parasSudoWrapper.sudoUnassignParachainSlot` (this clears any lease which downgrades the para back to a parathread).
This all relies on the existing `slots` pallet & lease mechanism, nothing fancy!
