...
> The Rococo `sudo` key is controlled by Parity tech at this time, so to have the operation required to get a slot, please on the [Subport repo `Rococo Request` issue template](https://matrix.to/#/#parachain-technical:matrix.parity.io) once you complete the above and are ready to connect!
