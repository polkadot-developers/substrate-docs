...
* You have completed the [Create your first Substrate blockchain]() and have the Substrate node template installed locally.
...
Before moving on, let's take a closer look at the options used to start the node template.
...
| `--alice` | Places the predefined keys for the Alice account in the node's keystore. With this setting, the `alice` account is used for block production and finalization.|
...
* `🏷 Local node identity is: 12D3KooWEyoppNCUx8Yx66oV9fJnriXwCcXwDDUA2kj6vnc6iDEp` shows the peer ID that the node started by Bob will need from Alice's node. This value was determined by the `--node-key` that was used to start Alice's node.
...
This tutorial shows you how to create a two-node network.
...
To create a new chain specification based on an existing specifications:
...
1. Modify `aura` field to specify the authority nodes that should be used for creating blocks by adding the Sr25519 SS58 address keys for each network participant.
...
1. Modify the `grandpa` field to specify the authorities that should be used for finalizing blocks by adding the Ed25519 SS58 address keys for each network participant. 
    ...
    "grandpa": {
      ...
      You might notice that there are two data values for the Grandpa `authorities` field. 
...
If you are creating a private blockchain network to share with other participants, you should ensure that only one person creates the chain specifiction and shares the resulting raw version of the specification—for example, the `customSpecRaw.json` file—with all of the other validators in the network. 
...
1. Add the `aura` secret key generated from the `key` subcommand by running a command similar to the following:
    ...
    Keep in mind that this command uses the second participant's secret key and that the `aura` key type is required to enable block production.
...
1. Add the `grandpa` secret key generated from the `key` subcommand to the local keystore by running a command similar to the following:
    ...
    Keep in mind that this command uses the second participant's secret key and that the `gran` key type is required to enable block finalization.
...
1. Restart the second blockchain node by running the following command:
    ...
    Notice that the command includes the `base-path` and `name` command-line options plus an additional `validator` option to specify that this node is a validator for the private network.
    Also note that all validators must be using _identical chain specifications_ to peer.
...
* [Accounts](https://wiki.polkadot.network/docs/learn-accounts) and [Key management](https://wiki.polkadot.network/docs/learn-account-generation) for more information about key generatiob and storage options.

* [Cryptography](https://docs.substrate.io/v3/advanced/cryptography/) for more information abot the signature schemes used for different keys.
