...
1. Type the password you used to generate keys.
...
1. Add the `grandpa` secret key generated from the `key` subcommand to the local keystore by running a command similar to the following:
   ...
   Block finalization requires at least two-thirds of the validators to add their keys to their respective keystore.
...
1. Restart the second blockchain node by running the following command:
   ...
   After both nodes have added their keys to their respective keystore (located under `/tmp/node01` and `/tmp/node02`) and been restarted, you should see the same genesis block and state root hashes.
...
- [Cryptography](/v3/advanced/cryptography/) for more information about the signature schemes used for different keys.
