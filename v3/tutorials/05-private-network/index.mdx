...
Each participant in the blockchain network should generate unique keys. 
...
Although you could use pre-generated keypairs to complete this tutorial, you would never use those keys in a production environment.
...
You can use the Polkadot-JS application to generate production keys.
However, you should never use a system connected to the internet to generate production keys.
Disconnecting from the internet is not required for this tutorial.

To use the Polkadot-JS application to generate keys:

1. Open the
[Polkadot-JS Explorer](https://polkadot.js.org/apps/?rpc=ws%3A%2F%2F127.0.0.1%3A9945#/explorer) in a web browser.

1. On the **Accounts** tab, click **Add account**. 

1. You do not need to provide a name, although you may if
