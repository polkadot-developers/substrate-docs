...
<TutorialObjective
  data={{
    textLineOne:
      'Create and edit a chain spec JSON file.',
...
Because you are joining a network that is already running, you will use the running node to bootstrap the new node.
The commands you use here are similar to the ones you used before, but with a few important differences.
...
1. Start a second local blockchain node using the `bob` account by running the following command:
    ...
    * This command includes the `--bootnodes` option and specifies a single boot node, the node started by `alice` using the following information:
      ...
      * The peer identifier for the running node that you made note of in the console output. In this case, `12D3KooWEyoppNCUx8Yx66oV9fJnriXwCcXwDDUA2kj6vnc6iDEp`.
      ...
      If you look at the console that started the second node, you should see output.

1. Open the
[Polkadot-JS Explorer](https://polkadot.js.org/apps/?rpc=ws%3A%2F%2F127.0.0.1%3A9945#/explorer) to verify the network is producing and finalizing blocks using the Polkadot-JS application.
    
      
      After you've verified that both nodes are running as expected, you can shut both nodes down. 

1. Shut down each node by pressing Control-C in each terminal.

## Generate your own keys

Now that you know how to start and connect running nodes using command-line options, you are ready to generate you own secret keys instead of using the predefined account keys. 
Each person who wants to participate in the blockchain should generate his or her own keys. 
There are several different ways you can generate keys, including:

* Subkey command-line program.

* 


Network participants can choose any of these methods to generate keys. 
Regardless of the method you choose, be sure to record the output because you will need the information later.
...
system should not be connected to the internet g such keys.
...
<Message
  ...
  text=
    ...
    generally desired ebapp like Polkadot JS Apps.
    "
/> 
...
you built earlier he `cargo build --release` command.
...
is very similar to hed a chain earlier, as Alice and Bob. It's important to start with
