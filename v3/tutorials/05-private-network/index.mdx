...
### Generate local keys using the node template<a name="generate-first-keypair"></a>
...
### Generate a second set of keys<a name="generate-second-keypair"></a>
...
1. Insert the `aura` secret key generated from the `key` subcommand by running a command similar to the following:
   ...
   Replace `<your-secret-seed>` with the secret phrase or secret seed for the first key pair that you generated in [Generate local keys using node template](#generate-first-keypair). 
...
1. Add the `aura` secret key generated from the `key` subcommand by running a command similar to the following:
   ...
   Replace `<second-participant-secret-seed>` with the secret phrase or secret seed that you generated in [Generate a second  key pair](#generate-second-keypair).
