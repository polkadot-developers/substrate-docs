...
using <ExternalLink url="https://github.com/paritytech/cumulus#readme">Cumulus</ExternalLink>.
The workshop is divided into 3 parts. At the end of this workshop, you will have learnt the process
of connecting any Substrate based chain to a relay chain and how to interact with it in a local
test network.

To learn more about parachains **before** continuing this workshop,
read about:
- <ExternalLink url="https://wiki.polkadot.network/docs/learn-parachains">Parachain Basics</ExternalLink>
- <ExternalLink url="https://polkadot.network/the-path-of-a-parachain-block/">The Path of a Parachain Block</ExternalLink>
...
\n
We will reference these assuming you have already underst all the steps involved, and
...
<TutorialObjective
data={{
  textLineOne: '2. Build Environment and Compilation',
  url: '#build-environment--compilation',
}}
/>
<TutorialObjective
data={{
  textLineOne: '3. Relay Chain Specification',
  url: '#relay-chain-specification',
}}
/>
<TutorialObjective
data={{
  textLineOne: '4. Starting the Relay Chain',
...
Compiling this project is a resource intensive process! We suggest compiling and running the
Parachain template on a machine with **no less than**:
...
connecting to. If you want to connect your parachian to a running relay network, you _must_ be
...
text={`
You **must** use these exact versions to ensure that you do not run into conflicts.
...
with the
<ExternalLink url="https://github.com/paritytech/polkadot/releases">latest release of Polkadot.</ExternalLink>
Please check with us in the
<ExternalLink url="https://matrix.to/#/#parachain-technical:matrix.parity.io">Parachain Technical matrix channel</ExternalLink>
when versions don't match there vs. what this workshop has listed, any issues you run into,
and when Rococo testnet resets occur.
...
### 2. Building the Polkadot Relay Chain Node
...
### 3. Building the Parachain Template

We will use the
<ExternalLink url="https://github.com/substrate-developer-hub/substrate-parachain-template"> Parachain Template </ExternalLink>
(similar but not identical to the
<ExternalLink url="https://github.com/substrate-developer-hub/substrate-node-template"> Node Template </ExternalLink>
) to launch our first parachain and make cross-chain asset transfers. Later, we will
use it as the starting point for developing our own parachains. In a different terminal window:
...
title={`Important`}
text={`
Keep in mind to always have one or more relay chain validator node running than
...
- <a href="/assets/tutorials/cumulus-workshop/chainspecs/rococo-custom-2-raw.json" download>
    rococo-custom-2-raw.json
  </a>
  : A two-validator relay chain with Alice and Bob as authorities. Useful for registering
  a single parachain. (
  <a href="/assets/tutorials/cumulus-workshop/chainspecs/rococo-custom-3-plain.json" download>
    plain chain spec
  </a>
  ).

- <a href="/assets/tutorials/cumulus-workshop/chainspecs/rococo-custom-3-raw.json" download>
...
- <a href="/assets/tutorials/cumulus-workshop/chainspecs/rococo-custom-4-raw.json" download>
  ...
  <a href="/assets/tutorials/cumulus-workshop/chainspecs/rococo-custom-4-plain.json">
    plain chain spec
  </a>
  ).

Plain chain spec files are in a more human readable and modifiable format for your inspection. They
can also be used to derive a new custom raw chain spec.
...
### 3. Convert Plain to Raw Chain Spec
